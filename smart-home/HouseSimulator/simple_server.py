...
class UserThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.__stop.is_set():
            ...
            if cmd == "d":
                if house.get_door() == "1":
                    house.set_door(False)
                else:
                    house.set_door(True)
                print("door is now {}".format(house.get_door()))
            elif cmd == "l":
                if house.get_light() == "1":
                    house.set_light(False)
                else:
                    house.set_light(True)
                print("light is now {}".format(house.get_light()))
            elif cmd == "p":
                if house.get_proximity() == "1":
                    house.set_proximity(False)
                else:
                    house.set_proximity(True)
...
def main():
    ...
    while True:
        ...
        print(
            "Waiting for house connection on {0}:{1}".format(
